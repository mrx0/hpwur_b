...
function raid_duplication_check($gym_id,$start,$end)
{
    ...
    $rs = my_query(
        "
        SELECT     raids.*,
                   gyms.lat, gyms.lon, gyms.address, gyms.gym_name, gyms.show_place,
...
function get_raid($raid_id)
{
    ...
    $rs = my_query(
        "
        SELECT     raids.*,
                   places.lat, places.lon, places.address, places.place_name, places.ex_gym,
                   users.name,
                   UNIX_TIMESTAMP(start_time)                      AS ts_start,
                   UNIX_TIMESTAMP(end_time)                        AS ts_end,
...
function get_user($user_id)
{
    ...
    // Unknown prof.
    if ($row['prof'] === NULL) {
        $msg .= 'Prof: ' . $GLOBALS['profs']['unknown'] . CR;

    // Known prof.
    } else {
        $msg .= 'Prof: ' . $GLOBALS['profs'][$row['prof']] . CR;
    }
 ...
 * @return array
 */
function raid_edit_opportunity_keys($place_id, $gym_first_letter, $arg, $admin = false){
...
//    );
    $keys = array(
        array(
            'text'          => 'Ферма '.EMOJI_EYE,
            'callback_data' => $place_id . '_' . $arg . ':raid_save:1'
        ),
        array(
            'text'          => 'Башня '.EMOJI_FORT,
            'callback_data' => $place_id . '_' . $arg . ':raid_save:2'
        ),
        array(
            'text'          => EMOJI_EYE.' + '.EMOJI_FORT,
            'callback_data' => $place_id . '_' . $arg . ':raid_save:3'
        )
    );
...
function raid_edit_gyms_first_letter_keys() {
    ...
    $rs = my_query(
            "
            SELECT UPPER(LEFT(place_name, 1)) AS first_letter
            FROM      places
            WHERE     show_place = 1
            GROUP BY LEFT(place_name, 1)
            "
        );
...
function raid_edit_gym_keys($first)
{
    ...
    $rs = my_query(
        "
        SELECT    places.id, places.place_name,
                  CASE WHEN SUM(raids.end_time > NOW() + INTERVAL 15 MINUTE) THEN 1 ELSE 0 END AS active_raid
        FROM      places
        LEFT JOIN raids
        ON        raids.place_id = places.id 
        WHERE     UPPER(LEFT(place_name, 1)) = UPPER('{$first}')
        AND       places.show_place = 1 
        GROUP BY  place_name 
        ORDER BY  place_name
        "
    );
...
//            [
//                'text'          => '+ ' . EMOJI_AUROR,
//                'callback_data' => $raid['id'] . ':vote_extra:auror'
//            ],
//            [
//                'text'          => '+ ' . EMOJI_MAGOZOOLOGIST,
//                'callback_data' => $raid['id'] . ':vote_extra:zoolog'
//            ],
//            [
//                'text'          => '+ ' . EMOJI_PROFESSOR,
//                'callback_data' => $raid['id'] . ':vote_extra:prof'
//            ]
//        ]
//    ];
...
//    ];
    ...
    $buttons_type = [
        [
            [
                'text'          => EMOJI_EYE.'+',
                'callback_data' => $raid['id'] . ':vote_dd:1'
            ],
            [
                'text'          => EMOJI_EYE.'-',
                'callback_data' => $raid['id'] . ':vote_dd:0'
            ]
        ]
    ];
...
//            ],
    ...
    // Raid is still running.
        ...
        $keys_default = explode(',', 'extra,teamlvl,type,time,pokemon,status');
        ...
        if(count($keys_UI_config) == count($keys_default) && count(array_diff($keys_UI_config, $keys_default)) == 0){
            ...
            // Default keys order
            $keys = array_merge($buttons_extra,$buttons_teamlvl,$buttons_type,$buttons_time,$buttons_pokemon,$buttons_status);
        }
    }
...
function send_response_vote($update, $data, $new = false)
{
    ...
    if ($raid) {
        ...
        // Edit the message.
        edit_message($update, getTranslation('raid_not_exists').EMOJI_MAN_SHRUGGING, [], ['disable_web_page_preview' => 'true'], true);
    }
...
function get_overview($update, $chats_active, $raids_active, $action = 'refresh', $chat_id = 0)
{
    ...
    foreach ($chats_active as $row) {
        ...
        */
        ...
        // Add to message.
        if ($att['count'] > 0) {
            $msg .= EMOJI_GROUP . '<b> ' . ($att['count'] + $att['extra_auror'] + $att['extra_valor'] + $att['extra_instinct']) . '</b> — ';
            $msg .= ((($att['count_auror'] + $att['extra_auror']) > 0) ? EMOJI_AUROR . ($att['count_auror'] + $att['extra_auror']) . '  ' : '');
            $msg .= ((($att['count_zoolog'] + $att['extra_zoolog']) > 0) ? EMOJI_MAGOZOOLOGIST . ($att['count_zoolog'] + $att['extra_zoolog']) . '  ' : '');
            $msg .= ((($att['count_prof'] + $att['extra_prof']) > 0) ? EMOJI_PROFESSOR . ($att['count_prof'] + $att['extra_prof']) . '  ' : '');
            $msg .= (($att['count_no_prof'] > 0) ? TEAM_UNKNOWN . $att['count_no_prof'] : '');
            $msg .= CR;
        }
...
function show_raid_poll($raid){
	...
	if ($raid['place_name']) {
		$msg .= getRaidTranslation('place') . ': ' . '<b>' . $raid['place_name'] . '</b>'.CR;
	}
...
//                    $count_att_time_poke_extrapeople = $current_att_time_poke['extra_auror'] + $current_att_time_poke['extra_zoolog'] + $current_att_time_poke['extra_prof'];
//                    $poke_count_auror = $current_att_time_poke['count_auror'] + $current_att_time_poke['extra_auror'];
//                    $poke_count_zoolog = $current_att_time_poke['count_zoolog'] + $current_att_time_poke['extra_zoolog'];
//                    $poke_count_prof = $current_att_time_poke['count_prof'] + $current_att_time_poke['extra_prof'];
//                    $poke_count_late = $current_att_time_poke['count_late'];
//
//                    // Add to message.
//                    $msg .= ' [' . ($current_att_time_poke['count'] + $count_att_time_poke_extrapeople) . '] — ';
//                    $msg .= (($poke_count_auror > 0) ? EMOJI_AUROR . $poke_count_auror . '  ' : '');
//                    $msg .= (($poke_count_zoolog > 0) ? EMOJI_MAGOZOOLOGIST . $poke_count_zoolog . '  ' : '');
//                    $msg .= (($poke_count_prof > 0) ? EMOJI_PROFESSOR . $poke_count_prof . '  ' : '');
//                    $msg .= (($current_att_time_poke['count_no_prof'] > 0) ? TEAM_UNKNOWN . ($current_att_time_poke['count_no_prof']) : '');
...
//            $msg .= ($row['prof'] === NULL) ? ($GLOBALS['profs']['unknown'] . ' ') : ($GLOBALS['profs'][$row['prof']] . ' ');
            ...
            //Если есть глаза / dark detectors
            if ($row['dd'] == 1){
                $msg .= SP.'+'.EMOJI_EYE;
            }
...
function show_raid_poll_small($raid){
    ...
    //Description - закомменчено, потому что в боте все равно не видно описание, после изменения
//    if ($raid['descr']){
//        $msg .= getTranslation('raid_description'). CR2;
//    }

    // Place Name
    if($raid['place_name']) {
        $msg .= '<b>' . $raid['place_name'] . '</b>' . CR;
    }

    // Address found.
    if ($raid['address']) {
        $msg .= '<i>' . $raid['address'] . '</i>' . CR;
    }
    ...
    if(!empty($raid['ts_start']) && !empty($raid['ts_end'])) {
        ...
        if($week_now == $week_start && $date_now == $date_raid) {
            // Output: Raid egg opens up 17:00
            // Показывается в сообщении, где сохраняется в боте
            $msg .= '<b>' . getTranslation('raid_start_time') . ' ' . unix2tz($raid['ts_start'], $raid['timezone'], 'd.m.Y H:i');
        } else {
            if($days_to_raid > 6) {
                // Output: Raid egg opens on Friday, 13 April (2018)
