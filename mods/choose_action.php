...
//choose_action.php
//Выбираем следующее действие после получения геолокации
...
//$keys = raid_edit_opportunity_keys($place_id, $place_first_letter);
//Выбираем дату
$keys = choose_action_keys($lat, $lon);
...
if(isset($update['message']['location'])) {
    // Build message.
    $msg = getTranslation('select_action');
...
// Answer forwarded location message from geo_create.
    ...
    // Edit the message.
    edit_message($update, getTranslation('select_action'), $keys);
